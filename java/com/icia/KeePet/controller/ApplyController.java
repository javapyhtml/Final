...
//    }
    ...
    @GetMapping("/applyForm/{bNum}")
    public ModelAndView applyForm(@PathVariable int bNum) {
        // 경로 변수 처리
        System.out.println("BNum: " + bNum);

        ModelAndView mav = new ModelAndView("Board/applyForm");  // applyForm.html 또는 해당 뷰 이름
        mav.addObject("bNum", bNum);  // bNum만 전달
...
//    }
    ...
    public ResponseEntity<List<ApplyDTO>> myapplyList() {

        List<ApplyDTO> applyList = applyService.getMyAppliedBoards();
        return new ResponseEntity<>(applyList, HttpStatus.OK);
    }

    @GetMapping("/applyList")
    public ResponseEntity<List<ApplyDTO>> applyList() {
        List<ApplyDTO> applyList = applyService.getAllAppliedBoards();
        return new ResponseEntity<>(applyList, HttpStatus.OK);
    }
