...
//      point.setAdddress(document.getRoadAddressName());
  ...
  /* 사용자 주소로 pointList값 가져오기
   */

  public static  List<Point> getPointByCoordinates(double x, double y) throws IOException, InterruptedException { //반환타입 List<Point> 로 수정
    HttpClient client = HttpClient.newHttpClient();
    String url = "https://dapi.kakao.com/v2/local/search/keyword.json";
    /*url += "&category_group_code=AT4";*/
    url += "&x=" + x;
    ...
    KakaoAddress kakaoAddress = objectMapper.readValue(responseBody, KakaoAddress.class);

    List<Document> documents = kakaoAddress.getDocuments();
    if (documents.isEmpty()) {
      return Collections.emptyList(); // 결과가 없을 경우 빈 리스트 반환 (null 대신)
    }

    // 변경: pointList 생성 및 Document -> Point 변환하여 리스트에 담기
    List<Point> pointList = new ArrayList<>();
    for (Document document : documents) {
      Point point = new Point();
      point.setX(document.getX());
      point.setY(document.getY());
      point.setName(document.getPlaceName());
      point.setPhone(document.getPhone() == null ? "전화번호 없음" : document.getPhone());
      point.setRoadAddress(document.getRoadAddressName());
      point.setJibunAddress(document.getAddressName());
      point.setId(document.getId());
      pointList.add(point);
    }
